                ...
                if (wrd2 not in dictionary) and (wrd2 not in dict_tmp):
                    ...
                    try:
                        if wrd2[-1] in ['s', 'S']:
                            ...
                            twrd2 = int(wrd2[1:])
                        elif wrd2[-2:] in ["th", "TH"]:
                            twrd2 = int(wrd2[:-2])
                        elif '-' in wrd2:
                            twrd2 = int(wrd2.split('-')[0])
                            #twrd2 = wrd2.replace('-', '')
                            
                        elif wrd2[0] in ["&"]:
                            twrd2 = 'and'
                            wrd2 = 'and'
                        elif re.search(r'[0-9][a-zA-Z]', wrd2) != None:
                            match = re.search(r'[0-9][a-zA-Z]', wrd2)
                            (beg, end) = match.span()
                            twrd2 = wrd2[:beg + 1]
                        else:
                            twrd2 = int(wrd2)
                        p = inflect.engine()
                        num2wrd = p.number_to_words(twrd2, andword='')
                        print("WHAT IS THIS: ", num2wrd)
